...
/**
 * This code makes the robot move tracking a line.
 * 
...
int main(int argc, char **argv)
{
  ...
  float y[5] = {-0.1347, -0.0586, 0, 0.0586, 0.1347};
  ...
  while (ros::ok() && count<=2000)
  {
      ...
      for(int i=0;i<5; i++)
      {
		  //ROS_INFO("x=%f y=%f z=%f i=%d", x, y[i], z,i);
		  t1= atan2(y[i],x);
	      k=z*z+d2*d2+(x*cos(t1)+y[i]*sin(t1))*(x*cos(t1)+y[i]*sin(t1))-l3*l3;
	      k1=2*x*cos(t1)*d2+2*y[i]*sin(t1)*d2;
	      k2=2*z*d2;
